...
# 🔑 從 GitHub Secrets 讀取金鑰 [cite: 648-650]
...
def fetch_google_news():
    ...
    try:
        ...
        for item in root.findall('./channel/item')[:10]:
            ...
            # 安全閥：防止 LINE API 報錯 [cite: 298, 871]
...
def get_gemini_summary(news_list):
    """AI 摘要生成 (修正模型清單 + 分類標題) [cite: 668-688]"""
    ...
    prompt = (
        f"以下是今日熱門新聞：\n{titles_text}\n\n"
        f"請以『{greeting}，為您帶來重點快報』開場。"
        "請根據新聞分段，每段必須以【分類名稱】開頭 (如：【政治焦點】)。"
        "⚠️ 格式要求：段落間空一行，嚴禁使用任何 Markdown 符號 (**)。"
    )

    client = genai.Client(api_key=GEMINI_API_KEY)
    # 🎯 修正後的階梯備援：優先使用 2026 旗艦模型 [cite: 683, 899]
    for model_name in ["gemini-2.5-flash", "gemini-2.0-flash-lite", "gemini-1.5-flash"]:
        try:
            print(f" 🤖 嘗試使用模型: {model_name} ...")
            response = client.models.generate_content(model=model_name, contents=prompt)
            # 確保內容淨化，移除星號 [cite: 686]
...
def update_pwa_data(news_list, summary):
    """[PWA 功能] 寫入 JSON [cite: 715-722, 942-959]"""
    try:
        tw_time = datetime.now(timezone(timedelta(hours=8))).strftime("%Y-%m-%d %H:%M")
